...
export const IconButton = ({children}:{children:ReactNode, href?:string}) => {
    return (
        <button  className="hover:scale-110 transition-all duration-300 cursor-pointer backdrop-blur-lg grid place-content-center size-[35px] bg-1/10 border rounded-[10px]">
            {children}
        </button>
    )
}
...
export const NetworkIcons = ({opacity}:{opacity?:number}) => {
    ...
    return (
        <div style={{opacity:opacity}} className=" hover:scale-107 transition-all duration-300 flex items-center gap-2.5 border bg-1/2 backdrop-blur-lg rounded-[17px] p-[5px] ">
            <IconButton>
                <Icon name="github"/>
            </IconButton>
            <IconButton>
